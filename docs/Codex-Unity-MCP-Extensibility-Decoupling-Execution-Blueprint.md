...
2. L2 下发 Unity 时必须同时生成 `legacy_stringified_action_data`（String，内容为 `JSON.stringify(action_data)`）。
3. L3 Handler 主路径仅使用 `legacy_stringified_action_data` 做类型化反序列化，不依赖 `JsonUtility` 直接吃开放对象。
4. `action_data` 在 Unity DTO 中仅作为兼容保留字段，不作为执行依据。

#### 8.1.3 阶段改动覆盖
1. 阶段一新增字段契约：`legacy_stringified_action_data: string`（推荐必填）。
...
1. 任意扩展动作都能在 L2 -> L3 传输中获得非空 `legacy_stringified_action_data`。
...
| R9-L2-02 | L2 调度透传 | `sidecar/src/application/unityDispatcher/runtimeUtils.js`、`sidecar/src/application/mcpGateway/mcpGateway.js` | 保留并透传 `action_data`；下发 Unity 时强制生成 `legacy_stringified_action_data` | L2->L3 传输中 `legacy_stringified_action_data` 非空且内容与原对象一致 |
...
| R9-L3-01 | L3 DTO 契约升级 | `Assets/Editor/Codex/Domain/SidecarContracts.cs` | 动作契约新增 `legacy_stringified_action_data`、能力上报 DTO（`unity.capabilities.report`） | Unity 端反序列化不丢字段，`JsonUtility` 可稳定接收 |
...
3. `JsonUtility` 风险可控：Unity 执行主路径依赖 `legacy_stringified_action_data`，不依赖开放对象直反序列化。
...
|---|---|---|---|---|
| D-R9-L2-01 | `R9-L2-05~08` 完成并稳定 | `sidecar/src/application/unityDispatcher/runtimeUtils.js` | 删除 `task_allocation` 回退读取；删除旧锚点字段（`target_object_path/target_object_id/object_id/parent_*`）在 runtime 下发路径的兼容映射 | runtime 下发仅包含标准锚点 + `legacy_stringified_action_data`，旧字段不再被读取 |
