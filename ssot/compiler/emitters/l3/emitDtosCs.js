...
function emitCreateFamilyContract(dictionary) {
  const definitions = normalizeDefinitions(dictionary);
  const createFamily = isPlainObject(definitions.create_family)
    ? definitions.create_family
    : {};
  const preCheckPolicy = isPlainObject(createFamily.pre_check_policy)
    ? createFamily.pre_check_policy
    : {};
  const ambiguityPolicy = isPlainObject(
    definitions.ambiguity_resolution_policy_contract
  )
    ? definitions.ambiguity_resolution_policy_contract
    : {};
  const nameCollision = isPlainObject(ambiguityPolicy.name_collision)
    ? ambiguityPolicy.name_collision
    : {};

  const enabled = preCheckPolicy.check_existing === true;
  const returnCandidates = preCheckPolicy.return_candidates === true;
  const fallbackDefaultPolicy =
    typeof nameCollision.default_policy === "string" &&
    nameCollision.default_policy.trim()
      ? nameCollision.default_policy.trim()
      : "fail";
  const defaultPolicy =
    typeof preCheckPolicy.on_conflict === "string" && preCheckPolicy.on_conflict.trim()
      ? preCheckPolicy.on_conflict.trim()
      : fallbackDefaultPolicy;
  const policyField =
    typeof preCheckPolicy.policy_field === "string" &&
    preCheckPolicy.policy_field.trim()
      ? preCheckPolicy.policy_field.trim()
      : "name_collision_policy";
  const allowedPoliciesRaw = Array.isArray(nameCollision.allowed_policies)
    ? nameCollision.allowed_policies
    : [];
  const allowedPolicies =
    allowedPoliciesRaw.length > 0
      ? allowedPoliciesRaw
      : ["fail", "suffix", "reuse"];
  const allowedPoliciesLiteral =
    allowedPolicies.length > 0
      ? `new[] { ${allowedPolicies
          .map((item) => `"${escapeCSharpString(item)}"`)
          .join(", ")} }`
      : "new string[0]";

  return `    public static class SsotCreateFamilyContract
    {
        public const bool PreCheckEnabled = ${enabled ? "true" : "false"};
        public const bool ReturnCandidates = ${returnCandidates ? "true" : "false"};
        public const string DefaultOnConflict = "${escapeCSharpString(defaultPolicy)}";
        public const string PolicyField = "${escapeCSharpString(policyField)}";
        public static readonly string[] AllowedOnConflictPolicies = ${allowedPoliciesLiteral};
    }`;
}
...
function emitDtosCs(dictionary) {
  ...
  context.blocks.push(emitCreateFamilyContract(dictionary));
