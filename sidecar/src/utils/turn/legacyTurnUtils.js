...
function mapTargetResolveReasonToPreconditionReason(errorCode) {
  if (errorCode === "E_TARGET_ANCHOR_CONFLICT") {
...
function looksLikeActionHandlerNotFoundMessage(message) {
  ...
  const hasHandlerToken =
    text.includes("handler") || text.includes("executor") || text.includes("registry");
  if (!hasHandlerToken) {
    return false;
  }
  return (
...
function normalizeUnityActionFailureCode(payload, fallback) {
  const source = payload && typeof payload === "object" ? payload : {};
  const explicitCode = normalizeErrorCode(source.error_code, "");
  if (explicitCode) {
    return explicitCode;
  }
  const errorMessage =
    typeof source.error_message === "string" && source.error_message.trim()
      ...
      : typeof source.message === "string" && source.message.trim()
        ? source.message.trim()
        : "";
  if (looksLikeActionHandlerNotFoundMessage(errorMessage)) {
    return "E_ACTION_HANDLER_NOT_FOUND";
  }
  return normalizeErrorCode(fallback, "E_ACTION_RESULT_MISSING_ERROR_CODE");
}
