  ...
  try {
    ...
    if (spawned && spawned.startedByRunner) {
      await runCase(report, "preflight_drain_stale_jobs", async () => {
        return await drainRunningJobs(args.baseUrl, {
          maxRounds: 24,
          sleepMs: 50,
        });
      });
    ...
    const submitFirst = await runCase(report, "submit_first_emit_pending", async () => {
      const writeGate = await issueWriteGate(args.baseUrl);
      const writeActions = buildSubmitActions(`stream_first_${runId}`);
    ...
    const submitSecond = await runCase(report, "submit_second_emit_queued", async () => {
      const writeGate = await issueWriteGate(args.baseUrl);
      const writeActions = buildSubmitActions(`stream_second_${runId}`);
      const res = await postJson(args.baseUrl, "/mcp/submit_unity_task", {
        thread_id: threadId,
    ...
    await runCase(report, "reconnect_cursor_receives_new_events", async () => {
      ...
      const writeGate = await issueWriteGate(args.baseUrl);
      const writeActions = buildSubmitActions(`stream_third_${runId}`);
    ...
    await runCase(report, "reconnect_last_event_id_header_with_window_meta", async () => {
      ...
      try {
        ...
        const writeGate = await issueWriteGate(args.baseUrl);
        const writeActions = buildSubmitActions(`stream_fourth_${runId}`);
...
function buildTestContext(maxDepth) {
  return {
    scene_revision: "rev_mcp_stream_runner",
    selection: {
      mode: "selection",
...
function buildSubmitActions(tag) {
  ...
  const className = `McpStreamRunner_${suffix}`;
