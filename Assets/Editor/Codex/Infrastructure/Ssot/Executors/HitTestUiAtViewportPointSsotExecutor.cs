...
{
    public sealed class HitTestUiAtViewportPointSsotExecutor
    {
        private readonly UnityRagReadService _readService;

        public HitTestUiAtViewportPointSsotExecutor()
            : this(new UnityRagReadService())
        {
        }

        internal HitTestUiAtViewportPointSsotExecutor(UnityRagReadService readService)
        {
            _readService = readService ?? new UnityRagReadService();
        }

        public SsotDispatchResponse Execute(HitTestUiAtViewportPointRequestDto request)
        {
            if (request == null)
            {
                return SsotRequestDispatcher.Failure(
                    "E_SSOT_SCHEMA_INVALID",
                    "hit_test_ui_at_viewport_point request payload is required.",
                    HitTestUiAtViewportPointRequestDto.ToolName);
            }

            var resolution = BuildResolution(request.resolution_width, request.resolution_height);
            ...
            var readRequest = new UnityHitTestUiAtViewportPointRequest
            {
                ...
                {
                    view = SsotExecutorCommon.Normalize(request.view),
                    coord_space = SsotExecutorCommon.Normalize(request.coord_space),
                    coord_origin = SsotExecutorCommon.Normalize(request.coord_origin),
                    x = (float)request.x,
                    y = (float)request.y,
                    resolution = resolution,
                    scope = scope,
                    max_results = request.max_results,
                    include_non_interactable = request.include_non_interactable,
            ...
            if (response == null)
            {
                return SsotRequestDispatcher.Failure(
                    ...
                    "hit_test_ui_at_viewport_point returned null response.",
                    HitTestUiAtViewportPointRequestDto.ToolName);
            }
            ...
            return SsotRequestDispatcher.Success(
                ...
                new SsotDispatchResultData
                {
                    ...
                    root_path = scope == null ? string.Empty : SsotExecutorCommon.Normalize(scope.root_path),
                    target_object_id =
