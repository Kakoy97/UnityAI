...
{
    public sealed class SetWorldRotationSsotExecutor
    {
        public SsotDispatchResponse Execute(SetWorldRotationRequestDto request)
        {
            if (request == null)
            {
                return SsotRequestDispatcher.Failure(
                    "E_SSOT_SCHEMA_INVALID",
                    "set_world_rotation request payload is required.",
            ...
            if (!IsFinite(request.x) || !IsFinite(request.y) || !IsFinite(request.z))
            {
                return SsotRequestDispatcher.Failure(
                    "E_SSOT_SCHEMA_INVALID",
                    "set_world_rotation requires finite x, y, z values.",
                    SetWorldRotationRequestDto.ToolName);
            }
            ...
            if (transform == null)
            {
                return SsotRequestDispatcher.Failure(
                    "E_TARGET_NOT_FOUND",
                    "set_world_rotation target transform is unavailable.",
                    SetWorldRotationRequestDto.ToolName);
            }

            Undo.RecordObject(transform, "SSOT set_world_rotation");
